...
pkgs <- list('tidyverse', 'table1', 'e1071', 'caret', 'ROCR', 'devtools', 'OptimalCutpoints')
...
set.seed(100)
start = Sys.time()
tune.out.radial=tune.svm(y~., data=dat_train, kernel ="radial", type='C-classification', 
...
# save object
saveRDS(tune.out.radial, file = "radial_svm.RDS")

summary(tune.out.radial)
best.linear.radial = tune.out.radial$best.model

# prediction on training dataset
svm.pred.radial <- predict(best.linear.radial, dat_train %>% select(!'y'), probability=TRUE)
confusionMatrix(svm.pred.radial, dat_train$y, positive = 'yes')$byClass

# on testing dataset
svm.pred.test.radial <- predict(best.linear.radial, dat_test %>% select(!'y'), probability=TRUE)
confusionMatrix(svm.pred.test.radial, dat_test$y, positive = 'yes')$byClass

# ROC curve using ROCR package
pr <- prediction(attr(svm.pred.test, "probabilities")[,2], dat_test$y)
prf <- performance(pr, measure="tpr", x.measure="fpr")

pr.radial <- prediction(attr(svm.pred.test.radial, "probabilities")[,2], dat_test$y)
prf.radial <- performance(pr.radial, measure="tpr", x.measure="fpr")

plot(prf, col = "lightsteelblue", print.auc=TRUE, print.auc.x = 0.5, print.auc.y = 0.3)
plot(prf.radial, add = TRUE, col = "orange", print.auc=TRUE, print.auc.x = 0.5, print.auc.y = 0.3)

lines(x = c(0,1), y = c(0,1),col="black")
performance(pr, measure = 'auc')@y.values %>% print() # AUC
performance(pr.radial, measure = 'auc')@y.values %>% print() # AUC
...
cv.ridgeLR <- cv.glm.logit.ridge(x, y, lambdas,fold=5)
print(Sys.time() - t0)
model.ridge <- glm.logit.ridge(x, y, lambda = cv.ridgeLR$lambda.min)
x.test <- model.matrix(y~., dat_test)[,-1]
probabilities <- predict.logistic(model.ridge, x.test)$prob
##
...
legend("bottom",
       ...
       col=c("goldenrod", "lightsteelblue", "orange"),
...
pr.1 <- prediction(predict.logistic(model.ridge, x)$prob, dat_train$y)

sens <- data.frame(x=unlist(performance(pr.1, "sens")@x.values), 
                   y=unlist(performance(pr.1, "sens")@y.values))
spec <- data.frame(x=unlist(performance(pr.1, "spec")@x.values), 
                   y=unlist(performance(pr.1, "spec")@y.values))
sens %>% ggplot(aes(x,y)) + 
  geom_line() + 
...
cutoff <- 0.125
predicted.classes <- ifelse(probabilities > cutoff, "yes", "no")
