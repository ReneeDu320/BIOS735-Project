...
Using the optimal cutoff, the confusion matrix for prediction on testing dataset and the ROC curve are shown below. 

```{r ResultRidge2}
confusionMatrix(factor(ridge.pred.test, levels = c('no', 'yes')), dat_test$y, positive = 'yes')$table
...
For SVM with linear kernel, the confusion matrix for prediction on testing dataset are shown below.

```{r linearSVM}
# display results
confusionMatrix(svm.pred.lin.test, dat_test$y, positive = 'yes')$table
```

For SVM with radial kernel, the confusion matrix for prediction on testing dataset are shown below.

```{r radialSVM}
# display results
confusionMatrix(svm.pred.rad.test, dat_test$y, positive = 'yes')$table
```

Table 3 shows all the model performance evaluation metrics using three methods on training and testing datasets.
